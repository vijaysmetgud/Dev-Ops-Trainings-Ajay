...
What kind of jobs you wrote script to automate :-
...
it scans the code or it code smells it checks the quality of code checks the bugs vulnerability 
...
5. install sonar qube plugin in jenkins under manage jenkins
   ...
   3. sonar qube server authentication token ---> to get authentication toke login to sonar qube dashboard or ui 
    go to --> My Account > Security > generate token copy that token and paste it here 
...
sonar location / where your project source(src) file there and build the job 
...
4. jenkins integrating with ibm amd aws cloud to login to cr registry build docker images and push to ibm or aws cloud 
6. integrating with kubernetes 
7. integrating with docker
8. integrating with ansible
9. integrating with packer 
10. please refer this file - jenkinsfile_docker-cloud-k8s.
